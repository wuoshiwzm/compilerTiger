...
static struct expty transExp(Tr_level level, S_table v, S_table t, A_exp e) {
	...
	case A_letExp:
		...
		int dec_num = 0;
		for (dlist = u.let.decs; dlist; dlist = dlist->tail) {
			if (dlist->head !- NULL) dec_num++;
		}

		// 2.2 初始化 Tr_exp 数组，保存表达式
		//     包括：每个变量的声明 , in 和 end 之间的 seqExp ，一共 dec_num+1 个
		trexp_arr = (Tr_exp*)checked_malloc((dec_num + 1) * sizeof(Tr_exp));
		int i = 0;
