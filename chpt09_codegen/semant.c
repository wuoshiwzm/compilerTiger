    ...
    case A_varDec:
      ...
      if (d->u.var.typ != NULL) {
        ...
        if (tt == NULL) {
          ...
          S_enter(v, d->u.var.var, E_VarEntry(access, Ty_Int()));
        }
        // 生成变量的  Tr_access
        S_enter(v, d->u.var.var, E_VarEntry(access, tt));
      ...
      S_enter(v, d->u.var.var, E_VarEntry(access, e.ty));
    ...
    case A_functionDec:
      ...
      for (fdecl = d->u.function; fdecl; fdecl = fdecl->tail) {
        ...
        for (fl = fdec->params, ts = formalTys; fl && ts; fl = fl->tail, ts = ts->tail) {
           ...
           * void *value : 形参); */
          S_enter(v, fl->head->name, E_VarEntry(Tr_allocLocal(level, fl->head->escape), ts->head));
        }
...
// 本地变量
static Tr_access varTrAccess(Tr_level level, bool escape) {
  return Tr_allocLocal(level, escape);
}
